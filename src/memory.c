...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-01-29 15:38:04 by ezeppa            #+#    #+#             */
/*   Updated: 2025-01-29 15:38:04 by ezeppa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "memory.h"

int	process_line_fdf(t_app *app, char *line, int x)
{
	char	**split;
	...
	while (split[x])
	{
		app->points[app->nb_rows * app->nb_cols + x]
			= (t_vec3){x, -app->nb_rows, ft_atoi(split[x])};
...
int	init_points(t_app *app, char *file)
{
	...
	while (line)
	{
		if (!process_line_fdf(app, line, 0))
			return (0);
		line = get_next_line(fd);
	}
	return (1);
